...
export default function OnlineAnnotationWorkbench() {
      ...
      <OnlineLayout showGlobalSwitch={false}>
        <div className="h-full bg-white flex items-center justify-center p-6">
          <div className="text-center">
            ...
            <Button size="sm" variant="outline" className="mt-4" onClick={() => navigate('/online-task-list')}>
              返回任务列表
            </Button>
          </div>
        </div>
  ...
  const exportRowsPreview = exportRowsForXlsx.slice(0, 5).map((row, idx) => ({
    序号: idx + 1,
    ...row,
  }))
      ...
      <div className="flex flex-col h-full">
        <div className="h-11 bg-white border-b border-gray-200 flex items-center justify-between px-4 gap-2 shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            <button onClick={() => navigate('/online-task-list')} className="text-gray-400 hover:text-gray-600">
